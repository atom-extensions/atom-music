...
const playListView = require("./atom-music-playlist-view");
...
class AtomMusicView {
			...
			<div className="atom-music">
				<div className="audio-controls-container" ref="container">
					<div className="btn-group btn-group-sm">
						...
						<button className="btn icon icon-playback-rewind" on={{click: this.back15}}></button>
						<button className="btn icon playback-button icon-playback-play" on={{click: this.togglePlayback}} ref="playbackButton"></button>
						<button className="btn icon icon-playback-fast-forward" on={{click: this.forward15}}></button>
					...
					<div className="btn-group btn-group-sm pull-right">
						...
						<button className="btn icon icon-list-ordered" on={{click: this.showPlayList}}>Search Playlist</button>
						...
						<label className="btn icon icon-file-directory" tabIndex="0" on={{keypress: this.clickOnSpaceBar}}>
							<input attributes={{style:"display: none;", type:"file", multiple:true, accept:"audio/*"}} on={{change: this.filesBrowsed}} />
							Open Music Files
						</label>
					</div>
					<div className="inline-block playing-now-container">
						...
						<div className="ticker" on={{click: this.changeTicker}}>
							<div ref="ticker"></div>
						</div>
					</div>
				</div>
				...
				<audio className="audio-player" on={{play: this.audioPlay, pause: this.audioPause, ended: this.songEnded}} ref="audio_player"></audio>
			</div>
		);
	}
	...
	showPlayList() {
		...
		if (this.playlistView) {
			this.playlistView.update(items);
		} else {
			this.playlistView = new playListView(this, items);
		}
