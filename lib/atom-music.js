...
const AtomMusic = {
	...
	activate(state) {
		...
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:toggle": () => this.atomMusicView.toggle()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:play-pause": () => this.atomMusicView.togglePlayback()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:toggle-shuffle": () => this.atomMusicView.toggleShuffle()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:show-playlist": () => this.atomMusicView.showPlayList()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:forward-15s": () => this.atomMusicView.forward15()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:backward-15s": () => this.atomMusicView.back15()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:next-track": () => this.atomMusicView.nextTrack()}));
		this.subscriptions.add(atom.commands.add("atom-workspace", {"atom-music:previous-track": () => this.atomMusicView.prevTrack()}));
	...
	deserializeAtomMusicView(state) {
		return new AtomMusicView(state.atomMusicViewState);
	},

	serialize() {
		return {
			deserializer: "atom-music/AtomMusicView",
			atomMusicViewState: this.atomMusicView.serialize()
		};
	}
};
